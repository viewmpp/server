...
func run() error {
	...
	projectHandler := project.NewHandler(s.Projects, limiter, templates, logger)
	userHandler := user.NewHandler(s.Users, s.Tokens, s.Sessions, limiter, mail, cfg.VerificationTTL, cfg.VerificationRC, cfg.EarlyAccessSeats, templates, &wg, logger)
